...
int main(){
    ...
    for(i = n-1; i>=0; i--){ //마지막 수부터 검사
        ...
        else if(F[arr[i]] < F[stack[top]]){ //더 많은 갯수가 있는 수가 있다면
            ans[i] = stack[top]; //ans[i]에 그 수를 넣어준다.
            push(arr[i]); //arr[i]를 스택에 넣어준다.
        }
        else{
            while (!isEmpty() && F[arr[i]] >= F[stack[top]]){//비어있지 않고, F[stack[top]]의 갯수가 F[arr[i]]의 갯수보다 작다면 
                pop(); //더 큰 수가 나올 때까지 pop해준다. 이런식으로 n-1 부터 0번째 수까지 탐색           }
            if(isEmpty()){ //비어져있다면
